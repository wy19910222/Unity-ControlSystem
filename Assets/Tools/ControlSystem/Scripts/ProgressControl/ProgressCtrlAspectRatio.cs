/*
 * @Author: wangyun
 * @CreateTime: 2023-07-09 16:18:13 991
 * @LastEditor: wangyun
 * @EditTime: 2023-07-09 16:18:13 997
 */
...
namespace Control {
	...
	public class ProgressCtrlAspectRatio : BaseProgressCtrlFloat {
		...
		protected override float Lerp(float from, float to, float t) {
			return basedSize ?
					AspectRatioUtils.LerpUnclampedBasedSize(from, to, t, GetComponent<AspectRatioFitter>()) :
					base.Lerp(@from, to, t);
		}

		protected override float GetT(float from, float to, float value) {
			return basedSize ?
					AspectRatioUtils.GetTimeBasedSize(from, to, value, GetComponent<AspectRatioFitter>()) :
					base.GetT(@from, to, value);
		}
	}
}